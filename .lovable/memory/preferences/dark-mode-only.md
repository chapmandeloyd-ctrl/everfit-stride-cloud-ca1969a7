...
type: preference
---
The app ships in dark mode for everyone. The `ThemeToggle` component in `src/components/ThemeToggle.tsx` returns `null` for any user whose email is not the owner (`apexbeastfast@yahoo.com`). Default theme in `src/main.tsx` is `dark` with `enableSystem={false}`.
