...
export function DailyPlaybook({ protocolId, accentColorClass = "text-primary" }: { protocolId: string; accentColorClass?: string }) {
  ...
  const isActiveToday = schedule.active_days.includes(today);
  const visibleItems = schedule.items.filter((it) => {
    if (!it.keto_type_filter || it.keto_type_filter.length === 0) return true;
    return todaysKeto && it.keto_type_filter.includes(todaysKeto);
  });

  const ketoMeta = todaysKeto ? KETO_TYPES[todaysKeto] : null;
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
        ...
        {DAY_LABELS.map((d, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-1">
              ...
              {isActive && !isToday && <div className="h-1 w-1 rounded-full bg-primary/60" />}
            </div>
          );
        })}
      </div>
      ...
      {ketoMeta && isActiveToday && (
        ...
        >
          <div>
            <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Today · {DAY_LONG[today]}</div>
      ...
      {/* Timeline */}
      {isActiveToday && visibleItems.length > 0 && (
