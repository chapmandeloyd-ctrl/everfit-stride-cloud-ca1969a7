...
import { ImagePlus, Trash2, Save, Sparkles } from "lucide-react";
...
export function WelcomeCardEditor() {
  ...
  const { data: welcomeCard, isLoading } = useQuery({
    queryKey: ["trainer-welcome-card", trainerId],
    queryFn: async () => {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const path = `${trainerId}/welcome-card.${ext}`;
  ...
  const removeImage = async () => {
    ...
    try {
      await supabase.from("trainer_welcome_cards").update({ image_url: null }).eq("id", welcomeCard.id);
      queryClient.invalidateQueries({ queryKey: ["trainer-welcome-card"] });
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          This card appears on every new client's home screen until you assign their first workout plan.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="rounded-lg overflow-hidden border">
          <div className="relative h-40 bg-gradient-to-br from-primary/20 to-primary/5">
            {welcomeCard?.image_url ? (
              <img
                src={welcomeCard.image_url}
                alt="Welcome card"
        ...
        <div className="flex gap-2">
          ...
          >
            ...
            {uploading ? "Uploading..." : welcomeCard?.image_url ? "Change" : "Upload"}
          </Button>
          {welcomeCard?.image_url && (
        ...
        <div className="space-y-2">
          ...
          <Textarea
            ...
            placeholder="Welcome to your fitness journey!"
            rows={2}
          />
