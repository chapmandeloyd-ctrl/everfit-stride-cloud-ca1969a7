...
/**
 * Shared "Protocol + Keto Synergy" demo content.
 * Used by both the standalone /synergy-card-demo route and the
 * Complete Plan page so the rich interactive card stays in sync.
 */
export const synergyContent: ProtocolCardContent = {
  overview: [
...
/**
 * Build a synergy DemoProtocol on the fly so we can reflect the
 * client's actual protocol + keto type pairing in the eyebrow/title/stats
 * while keeping the rich interactive content the same.
 */
export function buildSynergyProtocol(opts?: {
  protocolName?: string;
  ketoAbbr?: string;
  fastHours?: number;
  proteinPct?: number;
  ...
  const protocolName = opts?.protocolName ?? "72h Fast";
  const ketoAbbr = opts?.ketoAbbr ?? "HPKD";
  const fastHours = opts?.fastHours ?? 72;
  const proteinPct = opts?.proteinPct ?? 30;

  return {
    id: `synergy-${fastHours}h-${ketoAbbr}`,
    icon: Sparkles,
    ...
    subEyebrow: `${protocolName} × ${ketoAbbr}`,
    title: `${fastHours}h × ${ketoAbbr}`,
    titleSuffix: " — one metabolic switch",
    stats: [
      { value: `${fastHours}h`, label: "Fast", accentClass: "text-red-500" },
      { value: `${proteinPct}%`, label: "Protein" },
