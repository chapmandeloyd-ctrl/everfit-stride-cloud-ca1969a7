...
export function getPushSetupMessage(error?: unknown): string {
  ...
  if (isIOS && !isStandalone) {
    return "Install APEXBEAST-IF from Safari to your Home Screen first, then open it from that icon and tap Enable again.";
  }

  if (typeof Notification !== "undefined" && Notification.permission === "denied") {
    return "Notifications are blocked. Open iPhone Settings → Notifications → APEXBEAST-IF → Allow Notifications, then return and tap Enable.";
  }

  if (isIOS) {
    return "iPhone rejected the saved alert token. Open iPhone Settings → Notifications → APEXBEAST-IF, make sure Allow Notifications is on, then restart the Home Screen app and tap Enable again.";
  }
