...
export default function EditWorkout() {
  ...
  const filteredExercises = useMemo(() => {
    return exercises
      ?.filter((ex: any) => {
        ...
        if (videoFilter === "named" && !(!!ex.video_url && !isAutoGeneratedName(ex.name))) return false;
        if (videoFilter === "unnamed" && !(!!ex.video_url && isAutoGeneratedName(ex.name))) return false;
        if (equipmentFilter && !matchesEquipment(ex, equipmentFilter as any)) return false;
        if (muscleFilter && !matchesMuscle(ex, muscleFilter as any)) return false;
        return true;
      })
      ?.sort((a: any, b: any) => {
  ...
  }, [exercises, exerciseSearch, sortBy, videoFilter, equipmentFilter, muscleFilter]);

  const equipmentCounts = useMemo(() => {
    const base = exercises || [];
    const map: Record<string, number> = {};
    for (const opt of EQUIPMENT_OPTIONS) {
      map[opt.key] = base.filter(
        (ex: any) => matchesEquipment(ex, opt.key as any) && (!muscleFilter || matchesMuscle(ex, muscleFilter as any))
      ).length;
    }
    return map;
  ...
  const muscleCounts = useMemo(() => {
    ...
    for (const opt of MUSCLE_OPTIONS) {
      map[opt.key] = base.filter(
        (ex: any) => matchesMuscle(ex, opt.key as any) && (!equipmentFilter || matchesEquipment(ex, equipmentFilter as any))
      ).length;
    }
    return map;
          ...
          rendered.push(
            ...
            >
              ...
              <div className="px-3 py-3" onClick={(event) => event.stopPropagation()}>
                <Button type="button" variant="outline" className="w-full justify-start border-dashed text-muted-foreground" onClick={() => setActiveBlockId(group.id)}>
