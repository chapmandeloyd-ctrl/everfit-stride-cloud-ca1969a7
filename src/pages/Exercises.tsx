...
export default function Exercises() {
  ...
  const [nameFilter, setNameFilter] = useState<"all" | "named" | "needs_name">("all");
  ...
  const filteredExercises = exercises?.filter((exercise) => {
    ...
    const matchesName = nameFilter === "all" ||
      (nameFilter === "needs_name" && isAutoGeneratedName(exercise.name)) ||
      (nameFilter === "named" && !isAutoGeneratedName(exercise.name));

    return matchesSearch && matchesMuscle && matchesEquipment && matchesCategory && matchesTag && matchesName;
  }) || [];
  ...
  const needsNameCount = exercises?.filter(ex => isAutoGeneratedName(ex.name)).length || 0;
