...
export default function CreateWorkout() {
  ...
  // Counts for the filter chips (respect the other active row + search)
  const equipmentCounts = useMemo(() => {
    const base = exercises || [];
    const map: Record<string, number> = {};
    for (const opt of EQUIPMENT_OPTIONS) {
      map[opt.key] = base.filter(
        (ex) => matchesEquipment(ex, opt.key as any) && (!muscleFilter || matchesMuscle(ex, muscleFilter as any))
      ).length;
    }
    return map;
  ...
  const muscleCounts = useMemo(() => {
    ...
    for (const opt of MUSCLE_OPTIONS) {
      map[opt.key] = base.filter(
        (ex) => matchesMuscle(ex, opt.key as any) && (!equipmentFilter || matchesEquipment(ex, equipmentFilter as any))
      ).length;
    }
    return map;
